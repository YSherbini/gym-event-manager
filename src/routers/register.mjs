...
router.get('/registers', auth, async (req, res) => {
    try {
        const registers = await Register.find({ gymOwnerId: req.gymOwner._id }).populate('teams').populate('event')
        res.send(registers)
    } catch (err) {
        res.status(400).send(err)
    }
...
router.get('/registers/:id', auth, async (req, res) => {
    try {
        const register = await Register.findOne({ _id: req.params.id, gymOwnerId: req.gymOwner._id }).populate('teams')
...
router.delete('/registers/:id', auth, async (req, res) => {
    try {
        const register = await Register.findOneAndDelete({ _id: req.params.id, gymOwnerId: req.gymOwner._id })
        if (!register) {
            res.status(404).send()
        }
        res.send(register)
